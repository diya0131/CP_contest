...
ll get(ll v, ll tl, ll tr, ll l, ll r) {
    ...
    if (tl == l && tr == r) {
        ...
        push(v);
        ll tm = (tl + tr) / 2;
        ll mi = min(get(2 * v, tl, tm, l, min(tm, r)), get(2 * v + 1, tm + 1, tr, max(l, tm + 1), r));
...
int main() {
    ...
    while (true) {
        ...
        if (scanf("%c", &ch) == EOF) {
            ...
            for(i=0;i<8*n;i++){
                cout<<i _ tree[i][0] _ tree[i][1]<<endl;
            }

            if (qp.size() == 2) {
                if (qp[0] < qp[1]) {
                    cout << get(1, 0, 2 * n - 1, qp[0], qp[1]) << endl;
                } else {
                    cout << get(1, 0, 2 * n - 1, qp[0], n + qp[1]) << endl;
                }

            } else {
                if (qp[0] <= qp[1]) {
                    modify(1, 0, 2 * n - 1, qp[0], qp[1], qp[2]);
                    modify(1, 0, 2 * n - 1, n + qp[0], n + qp[1], qp[2]);
                    // modify(1, 0, 2 * n - 1, qp[0], qp[1], qp[2]);
                } else {

                    modify(1, 0, 2 * n - 1, qp[0], n + qp[1], qp[2]);
                    modify(1, 0, 2 * n - 1, 0, qp[1], qp[2]);
                    modify(1, 0, 2 * n - 1, n + qp[0], 2 * n - 1, qp[2]);
